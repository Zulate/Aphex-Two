...
var OutlineNode = class extends TempNode {
  ...
  constructor(scene, camera, params = {}) {
    ...
    this.edgeThicknessNode = nodeObject2(edgeThickness);
    this.edgeGlowNode = nodeObject2(edgeGlow);
    ...
    this._blurDirection = uniform2(new Vector2());
    this._depthTextureUniform = texture2(this._renderTargetDepthBuffer.depthTexture);
    this._maskTextureUniform = texture2(this._renderTargetMaskBuffer.texture);
    this._maskTextureDownsSampleUniform = texture2(this._renderTargetMaskDownSampleBuffer.texture);
    this._edge1TextureUniform = texture2(this._renderTargetEdgeBuffer1.texture);
    this._edge2TextureUniform = texture2(this._renderTargetEdgeBuffer2.texture);
    this._blurColorTextureUniform = texture2(this._renderTargetEdgeBuffer1.texture);
    this._visibleEdgeColor = vec32(1, 0, 0);
    this._hiddenEdgeColor = vec32(0, 1, 0);
    this._depthMaterial = new NodeMaterial();
    this._depthMaterial.fragmentNode = vec42(0, 0, 0, 1);
    ...
    this._textureNode = passTexture2(this, this._renderTargetComposite.texture);
  }
  /**
  ...
  updateBefore(frame) {
    ...
    _rendererState2 = RendererUtils.resetRendererAndSceneState(renderer, scene, _rendererState2);
    ...
    renderer.render(scene, camera);
    renderer.setRenderObjectFunction(_rendererState2.renderObjectFunction);
    this._selectionCache.clear();
    _quadMesh2.material = this._materialCopy;
    renderer.setRenderTarget(this._renderTargetMaskDownSampleBuffer);
    _quadMesh2.render(renderer);
    _quadMesh2.material = this._edgeDetectionMaterial;
    ...
    _quadMesh2.material = this._separableBlurMaterial;
    ...
    this._blurColorTextureUniform.value = this._renderTargetEdgeBuffer1.texture;
    this._blurDirection.value.copy(_BLUR_DIRECTION_X);
    _quadMesh2.material = this._separableBlurMaterial2;
    ...
    _quadMesh2.render(renderer);
    _quadMesh2.material = this._compositeMaterial;
    renderer.setRenderTarget(this._renderTargetComposite);
    _quadMesh2.render(renderer);
    RendererUtils.restoreRendererAndSceneState(renderer, scene, _rendererState2);
  }
  /**
  ...
  setup() {
    const prepareMask = () => {
      const depth3 = this._depthTextureUniform.sample(screenUV2);
      let viewZNode;
      if (this.camera.isPerspectiveCamera) {
        viewZNode = perspectiveDepthToViewZ2(depth3, this._cameraNear, this._cameraFar);
      } else {
        viewZNode = orthographicDepthToViewZ2(depth3, this._cameraNear, this._cameraFar);
      }
      const depthTest = positionView2.z.lessThanEqual(viewZNode).select(1, 0);
      return vec42(0, depthTest, 1, 1);
    };
    ...
    const edgeDetection = Fn2(() => {
      const resolution = textureSize2(this._maskTextureDownsSampleUniform);
      const invSize = vec22(1).div(resolution).toVar();
      const uvOffset = vec42(1, 0, 0, 1).mul(vec42(invSize, invSize));
      const uvNode = uv2();
      ...
      const diff1 = mul2(c1.r.sub(c2.r), 0.5);
      const diff2 = mul2(c3.r.sub(c4.r), 0.5);
      const d = vec22(diff1, diff2).length();
      const a1 = min2(c1.g, c2.g);
      const a2 = min2(c3.g, c4.g);
      const visibilityFactor = min2(a1, a2);
      const edgeColor = visibilityFactor.oneMinus().greaterThan(1e-3).select(this._visibleEdgeColor, this._hiddenEdgeColor);
      return vec42(edgeColor, 1).mul(d);
    });
    this._edgeDetectionMaterial.fragmentNode = edgeDetection();
    this._edgeDetectionMaterial.needsUpdate = true;
    const MAX_RADIUS = 4;
    const gaussianPdf = Fn2(([x, sigma]) => {
      return float2(0.39894).mul(exp3(float2(-0.5).mul(x).mul(x).div(sigma.mul(sigma))).div(sigma));
    });
    const separableBlur = Fn2(([kernelRadius]) => {
      const resolution = textureSize2(this._maskTextureDownsSampleUniform);
      const invSize = vec22(1).div(resolution).toVar();
      const uvNode = uv2();
      ...
      Loop2({ start: int2(1), end: int2(MAX_RADIUS), type: "int", condition: "<=" }, ({ i }) => {
        const x = kernelRadius.mul(float2(i)).div(MAX_RADIUS);
...
var outline = (scene, camera, params) => nodeObject2(new OutlineNode(scene, camera, params));
export {
